"""In-memory match.status hub for judge UI (single replica, A9)."""
...
logger = logging.getLogger("stk.judge_hub")


class JudgeHub:
    """Fan-out match.status to judge panel subscribers."""
    ...
    def publish(self, match_id: str, message: dict[str, Any]) -> int:
        ...
        for queue in queues:
            try:
                ...
                logger.exception("judge_hub_publish_failed match_id=%s", match_id)
        logger.info(
            "judge_hub_publish match_id=%s reason=%s subscribers=%s",
            match_id,
            message.get("reason"),
            delivered,
        )
